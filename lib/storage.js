...
export async function getWikiConfig() {
  ...
  return normalizeWikiConfig(r[WIKI_CONFIG_KEY]);
}

export async function saveWikiConfig(config) {
  await chrome.storage.sync.set({ [WIKI_CONFIG_KEY]: normalizeWikiConfig(config) });
}
