 ...
 * - migrateLegacy()：兼容旧版 xbe_tweets 数组数据，并给缺字段的旧数据补默认分类
 */

import { DEFAULT_CLASSIFICATION } from "./constants.js";
...
export async function saveTweets(tweets) {
  ...
  for (const t of tweets) {
    ...
    toSet[TWEET_PREFIX + t.id] = { ...DEFAULT_CLASSIFICATION, ...t, tags: t.tags || [] };
    index.push(t.id);
    existing.add(t.id);
    added++;
  }
...
export async function migrateLegacy() {
  ...
  if (Array.isArray(legacy[LEGACY_KEY]) && legacy[LEGACY_KEY].length) {
    ...
    for (const t of legacy[LEGACY_KEY]) {
      if (!t?.id || existing.has(t.id)) continue;
      toSet[TWEET_PREFIX + t.id] = { ...DEFAULT_CLASSIFICATION, ...t };
      index.push(t.id);
      existing.add(t.id);
    }
    toSet[INDEX_KEY] = index;
  ...
  for (const id of index) {
    ...
    if (!t) continue;
    const missing =
      t.category === undefined || t.nature === undefined || t.action === undefined || !Array.isArray(t.tags);
    if (missing) {
      patch[TWEET_PREFIX + id] = { ...DEFAULT_CLASSIFICATION, ...t, tags: t.tags || [] };
    }
  }
