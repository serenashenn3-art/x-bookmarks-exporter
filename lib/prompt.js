...
export function buildClassificationPrompt(tweet) {
  ...
  const typeLine = tweet.contentType ? `内容类型: ${tweet.contentType}（由页面结构判定，仅供参考）\n` : "";
...
export function normalizeClassification(raw, tweet) {
  ...
  // 宽松匹配：归一化空白与全角符号后精确匹配；失败再双向包含匹配
  // （AI 常返回 "AI / 技术"、"AI／技术"、"技术" 等变体，严格相等会全部降级为未分类）
  const norm = (s) => s.replace(/\s+/g, "").replace(/[／｜|]/g, "/").trim();
  const pick = (val, allowed, def) => {
    if (typeof val !== "string") return def;
    const v = norm(val);
    if (!v) return def;
    const exact = allowed.find((a) => norm(a) === v);
    if (exact) return exact;
    const hit = allowed.find((a) => v.includes(norm(a)) || norm(a).includes(v));
    return hit || def;
  };
